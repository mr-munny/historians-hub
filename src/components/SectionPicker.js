...
import { getSections } from "../services/database";

export default function SectionPicker({ onSelect, userName }) {
  ...
  const handleCodeSubmit = async () => {
    ...
    setCodeError(null);
    try {
      const { lookupTeacherByJoinCode } = await import("../services/database");
  ...
  return (
    ...
    >
      <div style={cardStyle}>
        ...
        >
          <Icon icon={compassOutline} width={28} style={{ color: theme.accentGold }} aria-hidden="true" />
          Historian's Hub
        </h1>
